...
const defaultSettings: any = {
  ...
  watermark_color: 'rgba(184, 184, 184)', //水印字体颜色
  watermark_fontsize: '18px', //水印字体大小
  watermark_alpha: 0.15, //水印透明度，要求设置在大于等于0.005
  watermark_width: 230, //水印宽度
  watermark_height: 230, //水印长度
  watermark_angle: 345, //水印倾斜度数
...
var domChangeCallback = function (records: any) {
  if (forceRemove) {
    forceRemove = false
    return
  }
...
var watermarkDom: any = hasObserver ? new MutationObserver(domChangeCallback) : null
var option: any = {
  childList: true,
  attributes: true,
  subtree: true
...
/*加载水印*/
var loadMark = function (settings: { monitor: undefined }) {
  ...
  if (!otdiv) {
    ...
    otdiv.setAttribute(
      'style',
      `pointer-events: none !important;display: block !important;position: fixed;top: 0;width:100vw;height:100vh;z-index:1000;
      `
    )
  ...
  defaultSettings.watermark_cols = parseInt(
    ((page_width - defaultSettings.watermark_x) /
      (defaultSettings.watermark_width + defaultSettings.watermark_x_space)) as any
  )
  var temp_watermark_x_space = parseInt(
    ((page_width - defaultSettings.watermark_x - defaultSettings.watermark_width * defaultSettings.watermark_cols) /
      defaultSettings.watermark_cols) as any
  )
  ...
  defaultSettings.watermark_rows = parseInt(
    ((page_height - defaultSettings.watermark_y) /
      (defaultSettings.watermark_height + defaultSettings.watermark_y_space)) as any
  )
  var temp_watermark_y_space = parseInt(
    ((page_height - defaultSettings.watermark_y - defaultSettings.watermark_height * defaultSettings.watermark_rows) /
      defaultSettings.watermark_rows) as any
  )
  ...
  for (var i = 0; i < defaultSettings.watermark_rows; i++) {
    ...
    for (var j = 0; j < defaultSettings.watermark_cols; j++) {
      ...
      mask_div.style['-ms-user-select' as any] = 'none'
      /*设置水印相关属性end*/
      ;(shadowRoot as any).appendChild(mask_div)
    }
  }
  ...
  if (minotor && hasObserver) {
    ...
    watermarkDom.observe((document.getElementById(defaultSettings.watermark_id) as any).shadowRoot, option)
  }
}
...
var removeMark = function () {
  ...
  _parentElement?.removeChild(watermark_element as any)
  // :ambulance: remove()
  // minotor 这个配置有些冗余
...
var globalSetting: { monitor: undefined }
/*初始化水印，添加load和resize事件*/
watermark.init = function (settings: { monitor: undefined }) {
...
/*手动加载水印*/
watermark.load = function (settings: { monitor: undefined }) {
  globalSetting = settings
  loadMark(settings)
}
...
//监听dom是否被移除或者改变属性的回调函数
var callback = function (records: string | any[]) {
  if ((globalSetting && records.length === 1) || (records.length === 1 && records[0].removedNodes.length >= 1)) {
    loadMark(globalSetting)
    return
  }
  ...
  var watermark_parent_element = document.getElementById(defaultSettings.watermark_parent_node)
  if (watermark_parent_element) {
    var newWidth: any = getComputedStyle(watermark_parent_element).getPropertyValue('width')
    var newHeight: any = getComputedStyle(watermark_parent_element).getPropertyValue('height')
...
var watermarkDom = new MutationObserver(callback) as any
var option: any = {
  childList: true,
  attributes: true,
  subtree: true,
