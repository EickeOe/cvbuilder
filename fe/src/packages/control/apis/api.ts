...
export const DOMAIN_NAME =
  ENV.VITE_APP_ENV === "local" ? "d1" : ENV.VITE_APP_ENV;
...
const fetchHeaderRef = {
  current: { "Content-Type": "application/json", appCode: "control" },
};
...
http.interceptors.response.use<any>(
  responseSuccessInterceptors,
  responseErrInterceptors
);
