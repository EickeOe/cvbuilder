import { Button, Card, notification, PageHeader, Popconfirm, Space } from 'antd'
import { useAsync, useAsyncFn } from 'react-use'
import XFormRender from '@/components/NXFormRender/XFormRender'
import formJson from './form.json'
import { useEffect, useRef } from 'react'
import { fetchTsListApi, postRosterApi } from '@/packages/developer/apis'
import { getSearchParams } from '@gcer/react-air'
import UserSelect from '@/components/UserSelect/UserSelect'
import { useRecoilValue } from 'recoil'
import { currentManageProductState } from 'developer/recoil'
...
export default function TsManage() {
    ...
    fetchTsListApi({
      ...
      const obj =
        data[0]?.dutyUsers.reduce((duty: any, current: any) => {
          duty[current.level] = current.userNames.map(({ userName, realName }: any) => ({
            value: userName,
            label: realName
          }))
          return duty
  ...
  return (
    <>
      <PageHeader
        ghost={false}
        title={`${currentManageProduct?.label}-值班管理`}
        extra={
          <>
      ...
      />
      <Card bordered={false}>
        <XFormRender ref={formRef} options={formJson as any} widgets={{ UserSelect }} />
      </Card>
    </>
  )
}
