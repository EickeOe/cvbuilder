...
export default function BasicLayout(props: Props) {
  ...
  const currentProductInfo = useCurrentApp()
  ...
  const hasMenus = useMemo(() => {
    if (currentProductInfo.menuConfig && currentProductInfo.menuConfig.enabled) {
      return true
    }
    return false
  }, [currentProductInfo])
  ...
  const [siderMenus, back, breadcrumbRoutes, selectedKeys] = useMemo(() => {
    ...
    const subMenu = menuInfo?.sub?.find?.((sys: any) => !!matchPath(pathname, { exact: false, path: sys.path }))
    ...
    const selectKeys = search(menus.find((menu: any) => new RegExp(`^${menu.path}`).test(pathname)) ?? {}, pathname)
  ...
  return (
    <div className={className.basicLayout}>
      <Layout>
        <BasicHeader />
        <Layout>
          {hasMenus && <BasicSider product={currentProductInfo} back={back} siderMenus={siderMenus} />}
